...
export default function ScrollStack({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    if (isCompactViewportRef.current) {
      cardsRef.current.forEach((card, i) => {
        ...
        const hasChanged =
          ...
          lastTransform.rotation !== 0 ||
          lastTransform.blur !== 0;

        if (hasChanged) {
          card.style.transform = "translate3d(0, 0, 0) scale(1) rotate(0deg)";
          card.style.filter = "";
          lastTransformsRef.current.set(i, resetTransform);
        }
      });
    ...
    const endElement = useWindowScroll
      ? (document.querySelector(`.${styles.end}`) as HTMLElement | null)
      : (scrollerRef.current?.querySelector(`.${styles.end}`) as HTMLElement | null);

    const endElementTop = endElement ? getElementOffset(endElement) : 0;

    cardsRef.current.forEach((card, i) => {
      const cardMeasureTarget = wrappersRef.current[i] ?? card;
      const cardTop = getElementOffset(cardMeasureTarget);
  ...
  const setupLenis = useCallback(() => {
    ...
    const lenis = new Lenis({
      wrapper: scroller,
      content: scroller.querySelector(`.${styles.inner}`) as HTMLElement,
      duration: 1.2,
      easing: (t: number) => Math.min(1, 1.001 - Math.pow(2, -10 * t)),
      smoothWheel: true,
      touchMultiplier: 2,
  ...
  useLayoutEffect(() => {
    ...
    const cards = Array.from(
      scope.querySelectorAll(`.${styles.card}`),
    ) as HTMLDivElement[];
    const wrappers = Array.from(
      scope.querySelectorAll(`.${styles.cardWrapper}`),
    ) as HTMLElement[];
    ...
    const applyViewportMode = () => {
      isCompactViewportRef.current = mediaQuery.matches;
      updateCardTransforms();
    };
